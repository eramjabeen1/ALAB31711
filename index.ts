...
class Vehicle {
    make: string; // brand like 'toyota'
    model: string; // model name like 'camry'
    status: "started" | "stopped"; // only allowed values
  
    constructor(make: string, model: string, status: "started" | "stopped") {
      this.make = make;
      this.model = model;
      this.status = status;
    }
    ...
    updateStatus(newStatus: "started" | "stopped"): void {
  ...
  function showStatus(vehicle: Vehicle): void {
  ...
  class MyCycle<T> {
    make: T | T[]; // can be a single value or a list of them
    model: T | T[];
  ...
  const numericCycle = new MyCycle<number>(1, 2);
  numericCycle.print();
  numericCycle.printAll();
  numericCycle.getTypeInfo();
  
  // testing with plain strings
  const simpleCycle = new MyCycle<string>("basic", "starter");
  simpleCycle.print();
  ...
  const bikeCollection = new MyCycle<string>(bikeBrands, bikeModels);
